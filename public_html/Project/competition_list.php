...
$stmt = $db->prepare("SELECT Shootup_Comps.id, title, min_participants, current_participants, current_reward, expires, creator_id, min_score, join_cost, IF(competition_id is null, 0, 1) as joined,  CONCAT(first_place,'% - ', second_place, '% - ', third_place, '%') as place FROM Shootup_Comps
JOIN Shootup_Payout_Options on Shootup_Payout_Options.id = Shootup_Comps.payout_option
LEFT JOIN (SELECT * FROM Shootup_UserComps WHERE user_id = :uid) as uc ON uc.competition_id = Shootup_Comps.id WHERE expires > current_timestamp() AND did_payout < 1 AND did_calc < 1 ORDER BY expires asc");
$results = [];
try {
                        ...
                        <?php foreach ($results as $row) : ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php endif; ?>
                                    <a class="btn btn-secondary" href="competition_view.php?id=<?php se($row, 'id'); ?>">View</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else : ?>
